...
Also run View() to see more about the structure of model_fam and see which data are stored in it
```{r}
View(model_fam)
```
...
```{r}
ca_df %>% filter(large.residual == TRUE)
```
...
```{r}
ca_df %>% 
  ggplot(aes(x = fitted, y = studentize)) +
  geom_point() +
