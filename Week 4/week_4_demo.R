...
seniorSurvey.df <- read_csv("./data/seniorsurvey.csv") # replace text in the parentheses with your file name
...
psych::describe(seniorSurvey.df)
...
seniorSurveyScales.df <- seniorSurvey.df %>% select(SJ1:DA5)
...
mydf <- tidyr::replace_na(mydf, list(ParticipateCService = 0, ParticipateStudyAbroadSemester = 0, ParticipateServiceL = 0))
...
# instead
my_spearman_correlations <- mydf %>% select(SJCa,SJCh,DA) %>% cor(method="spearman")
print(my_spearman_correlations)

# If we need p values though, we need to change to something else -- corr.test

myresults <- corr_test(mydf$SJCa,mydf$DA)
...
qqnorm(mydf$SJCa, frame = FALSE)
qqline(mydf$SJCa, col = "steelblue", lwd = 1.5)

mydf %>% ggplot(aes(x = SJCa)) +
  geom_histogram()
